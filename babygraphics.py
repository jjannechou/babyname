...
def get_x_coordinate(width, year_index):
    ...
    return x_coordinate+GRAPH_MARGIN_SIZE

def get_y_coordinate(height, rank):
    y_coordinate = (height-GRAPH_MARGIN_SIZE*2)* int(rank)//MAX_RANK
    return y_coordinate+GRAPH_MARGIN_SIZE
...
def draw_names(canvas, name_data, lookup_names):
    ...
    for name in lookup_names:
        ...
        for i in range(len(YEARS)-1):
            ...
            if year in year_d:
                start_y = get_y_coordinate(CANVAS_HEIGHT, year_d[year])
                rank = year_d[year]
            else:
            ...
            if year_next in year_d:
                end_y = get_y_coordinate(CANVAS_HEIGHT, year_d[year_next])
            else:
                end_y = CANVAS_HEIGHT-GRAPH_MARGIN_SIZE
            canvas.create_line(start_x, start_y, end_x, end_y, fill=color,  width = LINE_WIDTH)
            canvas.create_text(start_x+TEXT_DX, start_y, text=name+rank, anchor=tkinter.NW, fill=color)
        end_x = get_x_coordinate(CANVAS_WIDTH, len(YEARS) - 1)
        if str(YEARS[-1]) in year_d:
            end_y = get_y_coordinate(CANVAS_HEIGHT, year_d[str(YEARS[-1])])
            rank = year_d[str(YEARS[-1])]
        else:
            end_y = CANVAS_HEIGHT - GRAPH_MARGIN_SIZE
            rank = '*'
        canvas.create_text(end_x + TEXT_DX, end_y, text=name+rank, anchor=tkinter.NW, fill=color)
        color_index += 1
